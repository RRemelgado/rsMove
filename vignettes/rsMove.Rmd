...
<p align="justify" style="line-height:200%;">
In addition, `rsMove` provides some example remote sensing data. This consists of three Tasseled Cap Brightness (TCb) images derived with Landsat surface reflectances. This data was provided by the United States Geological Survey ([USGS](https://landsat.usgs.gov/)) through the Earth Resources Observation and Science (EROS) Center Science Processing Architecture ([ESPA](https://espa.cr.usgs.gov/)). Moreover, we provide a land cover information (`landCover`) that overlaps with the `shortMove` dataset. This data was extracted from the european [Urban Atlas](https://land.copernicus.eu/local/urban-atlas). This data can be accessed as shown below.
...
<p align="justify" style="line-height:200%;">
Once we select adequate satellite data we can consider which variables to derive from it. However, the amount of potentially relevant variables is endless. So which variables to chose? `rsMove` cannot tell its user which variables to use. But it can help on the decision process. To achieve this, the package provides `moveSeg()`, `timeDir()` and `spaceDir()`.
</p>
<p align="justify" style="line-height:200%;">
`moveSeg()` helps the user visualize the distribution of the observed time in relation to changing environmental conditions. First, the function queries a `raster` for a `SpatialPoints` object. Then, the function divides consecutive observations in temporal segments defined as sequences of points collected over similar environmental conditions. Let's apply the function over the `shortMove` movement data and use the `landCover` land cover data as environmental information. As we can see below, we need additionally to specify the data type of `landCover`. Doing so will force `moveSeg()` to identify a new temporal segment every time a change in land cover occurs. 
</p>
...
<p align="justify" style="line-height:200%;">
Now, let's apply `moveReduce()`. The output, shown below, is a `SpatialPointsDataFrame` object with the reduced sample set. As we can see in the output, the large concentration of samples is preserved as a result of the frequent return trips to the nest. The nest is clearly visible in 
</p>


```{r}
reduced.samples <- moveReduce(xy=shortMove, obs.time=obs.time, img=ndvi[[1]])
```

```{r, fig.show='hold', echo=FALSE}
plot(ndvi[[1]], ext=shortMove)
points(shortMove)
points(reduced.samples$points, col="red")
plot(reduced.samples$total.time)
```


```{r, echo=FALSE}
knitr::kable(head(reduced.samples$points, 5))
```
