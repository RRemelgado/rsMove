...
<p align="justify" style="line-height:200%;">
In addition, `rsMove` provides two remote sensing datasets. First, the package offers three Normalized Difference Vegetation Index (NDVI) images derived with Landsat surface reflectances. This data was acquired though the Earth Resources Observation and Science (EROS) Center Science Processing Architecture ([ESPA](https://espa.cr.usgs.gov/)), a service of the United States Geological Survey ([USGS](https://landsat.usgs.gov/)). Second, the user can find a land cover map (`landCover`) with 30 m resolution derived from the European [Urban Atlas](https://land.copernicus.eu/local/urban-atlas). Both datasets overlap with the `shortMove` dataset and can be accessed as shown below.
</p>
...
<p align="justify" style="line-height:200%;">
The function identified 21 unique sample regions. The code below plots the region shapefile showing the relation between the original samples (in black) and the resulting regions (in red). The results suggestz that, from the 21 regions, only a few (4-5) are represented by a large number of samples, possibly representive locations where the animal remained during relatively longer periods of time.
</p>
...
<p align="justify" style="line-height:200%;">
Using the output of `hotMove()`, we can then use `hotMoveStats()` to deconstruct the composition of the regions. This will help us make a more informed decision about which regions to choose as potential test sites. We will provide `hotMoveStats()` with the sample indices acquired through `hotMove()` and the observation date of each sample. The later will help `hotMoveStats()` evaluate the temporal composition of each region. Below, we can see the function call as well as the final report.
</p>
...
<p align="justify" style="line-height:200%;">
Now that we know where to focus, we need to decided which satellite data to use. Assuming we want to rely NDVI data, let's use `sMoveRes()` and `tMoveRes()` to quantify the data loss with a few common, open-access sensors, namely Sentinel-2 (10 m), Landsat (30 m) and MODIS (250 m).
</p>
