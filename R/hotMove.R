...
#' @param pxr Resolution of the grid used for region labelling. Unir depends on the projection of the input.
...
#' @param cs If shp=TRUE then a valid CRS indicating the projection of the input coordinates is required.
...
#' @import sp, grDevices
#' @seealso \code{\link{sampleMove}}, \code{\link{cRS}}
#' @examples \dontrun{
#' # reference data
...
#' 
#' # extract regions
#' hm <- hotMove(x=whiteStork$lon, y=whiteStork=lon, pxr=0.01, shp=T, cs=outProj)
#' 
#' # plot shapefile
#' plot(hm$polygons)
#' }
...
hotMove <- function(x=x, y=y, pxr=pxr, shp=F, cs=NULL) {
  ...
  if (!exists(pxr)) {stop('error: a value is required for pxr')}
  if (shp==T) {
  ...
  if (shp==T) {
    ...
    # merge polygons
    uv <- unique(r)
    shp <- sp::SpatialPolygons(lapply(uv, pc), proj4String=cs)
