...
#' @param time Valid "POSIXlt" or "POSIXct" type vector with the time of observation for each record
#' @param method Selects the appropriate method to estimate the distance between consecutive points. if 'm' it estimates the ecludian distance. If 'deg' it uses the haversine formula. Default is 'deg'.
...
#' output <- sampleMove(whiteStork$lon, whiteStork$lat, whiteStork$time, 7, 'm')
#'
#' # with reference grid
#' refGrid<- data('constanceGrid')
#' output <- sampleMove(whiteStork$lon, whiteStork$lat, whiteStork$time, 7, 'm', refGrid)
#' 
#' # compare original vs new samples
#' plot(whiteStork$lon, whiteStork$lat, col="black", pch=16)
#' points(x, y, col="red", pch=15)
#' }

#-------------------------------------------------------------------------------------------------------------------------------#

sampleMove <- function(x=x, y=y, tim=time, error=error, method=method, tUnit=tUnit, layer=NULL) {
  ...
  if (!is.null(layer)) {if (!grDevices::is.raster(layer)) {stop('error: layer is not a valid raster object')}}
